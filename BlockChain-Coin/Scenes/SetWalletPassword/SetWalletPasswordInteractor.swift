...
class SetWalletPasswordInteractor: SetWalletPasswordBusinessLogic {
    ...
    func setPassword(request: SetWalletPasswordRequest) {
        ...
        if let seed = localWalletWorker.generateSeed(), let keyPair = localWalletWorker.generateKeyPair(seed: seed) {
            ...
            remoteWalletWorker.addWallet(keyPair: keyPair, uuid: uuid, secretKey: nil, password: nil, address: nil, details: nil, completion: { [weak self] result in
                switch result {
                case .success(let address):
                    ...
                    self?.remoteWalletWorker.getBalanceAndTransactions(wallet: wallet, password: wallet.password ?? "", completion: { detailsResult in
                        ...
                        case .success(let details):
                            self?.localWalletWorker.addWallet(keyPair: keyPair, uuid: uuid, secretKey: nil, password: request.form.password, address: address, details: details, completion: { localResult in
                                switch localResult {
                                case .success:
